# experiments.py: What does it look for?
#   1. GROUND TRUTH  : how close do our methods get to the TRUE optimum?
#   2. CONVERGENCE   : does annealing approach the optimum with more time?
#   3. COMPLEXITY    : does measured time AND space match the theory?
...
def _anneal_from_random(p, iterations, seed):
    rng = random.Random(seed)
    cur = _random_order(p, rng)
    cur_ms = serial_sgs(p, order=cur).makespan
    best = cur_ms
    temp = 2.0
    for _ in range(iterations):
        cand = _swap(p, cur, rng)
        cms = serial_sgs(p, order=cand).makespan
        d = cms - cur_ms
